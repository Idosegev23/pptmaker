...
## מיפוי 7 שלבים ל-8 ה-layouts:

- שקף 1 (פתיחה): hero-cover
- שלב 1 (מותג): full-bleed-image-text
- שלב 2 (מטרות): three-pillars-grid או numbered-stats
- שלב 3 (קהל): split-image-text
- שלב 4 (תובנה): centered-insight — dataPoint + dataLabel + **source חובה**
- שלב 5 (אסטרטגיה): 2-3 שקפים — full-bleed-image-text + split-image-text + three-pillars-grid
- שלב 6 (קריאייטיב): full-bleed-image-text עם סיפור + split-image-text עם "איך"
- שלב 7 (תוצרים): numbered-stats (deliverables) + influencer-grid (אם יש משפיענים)
- סיום: closing-cta
...
function buildUserPrompt(input: GenerateStructuredInput): string {
  ...
  lines.push(
    `\n## פלט: JSON מובנה של StructuredPresentation, 11-14 שקפים, 7 שלבי פיצוח מלאים. insight עם source אמיתי. creative עם רפרנס-עולם. JSON בלבד.`,
  )
