...
export default function ResearchPage() {
  ...
  // Save research to user's Google Drive as a Google Doc
  const saveToGoogleDrive = useCallback(async () => {
    if (!researchResults || savingToDrive) return
    setSavingToDrive(true)
    try {
      const res = await fetch('/api/research/save-to-drive', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const result = await res.json()
      if (!res.ok) {
        alert(result.hint || result.error || 'שמירה ל-Drive נכשלה')
        return
      }
      ...
      alert(`שמירה ל-Drive נכשלה: ${err instanceof Error ? err.message : 'שגיאה'}`)
    } finally {
      setSavingToDrive(false)
    }
  }, [researchResults, savingToDrive, documentId])
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-3 flex-wrap">
                <button
                  ...
                  title={driveLink ? 'פתח מסמך קיים ב-Drive' : 'שמור מחקר ל-Google Drive'}
                >
                  {savingToDrive ? (
                  ...
                  {savingToDrive ? 'שומר ל-Drive...' : driveLink ? 'פתח ב-Drive' : 'שמור ל-Google Drive'}
                </button>
                <button
