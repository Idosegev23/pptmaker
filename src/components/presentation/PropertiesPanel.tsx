...
function TextProperties({ element, onChange, onAIRewrite }: {
  element: TextElement
  ...
  return (
    <div className="space-y-3">
      ...
      {onAIRewrite && (
        <button
          onClick={onAIRewrite}
          className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-purple-500/10 border border-purple-500/20 rounded-lg text-purple-300 text-xs hover:bg-purple-500/20 transition-colors"
        >
          <span>✨</span>
          <span>שכתב עם AI</span>
        </button>
      )}
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          ...
          <input
            ...
            onChange={(e) => onChange({ fontSize: Math.max(8, Math.min(200, parseInt(e.target.value) || 16)) })}
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => onChange({ fontWeight: parseInt(e.target.value) as FontWeight })}
            className="w-full bg-white/5 border border-white/10 rounded px-2 py-1.5 text-white text-xs focus:outline-none focus:border-white/30"
          >
        ...
        <div>
          ...
          <div className="flex gap-1">
            ...
            <input
              ...
              value={element.color}
              onChange={(e) => onChange({ color: e.target.value })}
              className="flex-1 bg-white/5 border border-white/10 rounded px-2 py-1.5 text-white text-xs focus:outline-none focus:border-white/30 font-mono"
              dir="ltr"
            />
          </div>
        </div>
    ...
    <div className="space-y-3">
      ...
      {onReplace && (
        <div className="grid grid-cols-3 gap-1.5">
          <button
            onClick={() => onReplace('upload')}
            className="px-2 py-2 bg-white/5 border border-white/10 rounded-lg text-gray-300 text-[11px] hover:bg-white/10 transition-colors"
          >
            📁 העלה
          </button>
          <button
          ...
          <button
            onClick={() => onReplace('ai')}
            className="px-2 py-2 bg-purple-500/10 border border-purple-500/20 rounded-lg text-purple-300 text-[11px] hover:bg-purple-500/20 transition-colors"
          >
            ✨ AI
          </button>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          ...
          <select
            ...
            onChange={(e) => onChange({ objectFit: e.target.value as ImageElement['objectFit'] })}
            className="w-full bg-white/5 border border-white/10 rounded px-2 py-1.5 text-white text-xs focus:outline-none focus:border-white/30"
          >
      ...
      <div>
        ...
        <input
          ...
          className="w-full bg-white/5 border border-white/10 rounded px-2 py-1.5 text-white text-[10px] focus:outline-none focus:border-white/30 font-mono"
          dir="ltr"
        />
      </div>
    </div>
    ...
    <div className="space-y-3">
      ...
      <div>
        ...
        <div className="flex gap-1">
          <input
            type="color"
            value={element.fill.startsWith('#') ? element.fill : '#333333'}
...
function SlideProperties({ slide, onBackgroundUpdate, onRegenerateSlide }: {
  slide: Slide
  onBackgroundUpdate: (bg: SlideBackground) => void
  onRegenerateSlide?: () => void
}) {
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <input
          ...
          onChange={(e) => onBackgroundUpdate({ type: bgType, value: e.target.value })}
          placeholder={bgType === 'solid' ? '#1a1a2e' : bgType === 'gradient' ? 'linear-gradient(...)' : 'URL'}
          className="w-full bg-white/5 border border-white/10 rounded px-2 py-1.5 text-white text-xs focus:outline-none focus:border-white/30 font-mono"
          dir="ltr"
        />
      </div>
      ...
      {bgType === 'solid' && (
        <input
          ...
          onChange={(e) => onBackgroundUpdate({ type: 'solid', value: e.target.value })}
          className="w-full h-10 rounded border border-white/10 cursor-pointer bg-transparent"
        />
      )}
      ...
      {onRegenerateSlide && (
        <>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-3 py-2.5 bg-purple-500/10 border border-purple-500/20 rounded-lg text-purple-300 text-xs hover:bg-purple-500/20 transition-colors"
          >
            <span>✨</span>
            <span>צור שקף מחדש עם AI</span>
          </button>
        </>
      )}
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <h3 className="text-gray-400 text-[11px] font-medium uppercase tracking-wider pt-2 first:pt-0">
      {children}
    </h3>
  )
}
